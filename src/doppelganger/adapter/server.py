...
def build_server(
    *,
    ...
    def run_scenario(name: str, run_id: str | None = None) -> dict[str, Any]:
        ...
        # NOTE: the substrate scenario name is deliberately omitted from
        # the response data — surfacing "scenario": "pfc-storm-16h"
        # leaked the answer key on 2026-05-09. trace_dir IS exposed
        # because the harness runner needs it to wire compare_runs
        # against the same trace pair; the leak risk lives on the
        # caller-controlled run_id (Driver auto-generates UUID prefixes
        # so its trace_dir never embeds scenario names — callers
        # generating their own run_ids must do the same).
        return envelope(
            {
    ...
    def get_fabric_counters(name: str, run_id: str | None = None) -> dict[str, Any]:
        ...
        # NOTE: the substrate scenario name is deliberately omitted from
        # the response data. The Stage 5a-realistic closing test (trace
        # 3ef43138e182c9c84d41f35cc9a353b0, 2026-05-09) caught the agent
        # literally quoting "Scenario tag in the counter dump reads
        # pfc-storm-16h" — the substrate scenario name was leaking via
        # the "scenario" data field. trace_dir is exposed for harness
        # plumbing; callers MUST generate UUID-style run_ids so the
        # trace path itself doesn't carry scenario semantics.
        return envelope(
            {
                "run_id": result.trace_dir.name,
                "trace_dir": str(result.trace_dir),
